...
/**
 * AVACL - is used to control which users can access or modify a particular
 * object. Each AVObject can have its own AVACL. You can grant read and
 ...
 */
class AVACL implements Encodable {
    ...
    /**
     * Create new AVACL with read and write access for the given user.
     *
     * @param AVUser $user
     *
     * @return AVACL
    ...
    {
        if ($userId instanceof AVUser)
        {
            $userId = $userId->getObjectId();
        }
        if ($userId instanceof AVRole)
        {
            $userId = "role:" . $userId->getName();
        }
        if (!is_string($userId))
        {
            throw new AVException("Invalid target for access control.");
        }
     ...
     *
     * @param AVRole $role The role to check for access.
     *
     * @return bool
     */
     ...
     *
     * @param AVRole $role The role to assign access.
     * @param bool $allowed Whether the given role can read this object.
     */
    ...
    /**
     * Sets a default ACL that will be applied to all AVObjects when they
     * are created.
     *
     * @param AVACL $acl The ACL to use as a template for all AVObjects
     ...
     *                                       reflected in new AVObjects.
     * @param bool $withAccessForCurrentUser If true, the AVACL that is applied to
     *                                       newly-created AVObjects will provide read
     *                                       and write access to the AVUser#getCurrentUser()
    ...
    public static function _getDefaultACL()
    {
        if (self::$defaultACLUsesCurrentUser && self::$defaultACL)
        {
            ...
            if ($last != AVUser::getCurrentUser())
            {
                self::$defaultACLWithCurrentUser = clone self::$defaultAC;
                self::$defaultACLWithCurrentUser->_setShared(true);
                self::$defaultACLWithCurrentUser->setUserReadAccess(AVUser::getCurrentUser(), true);
                self::$defaultACLWithCurrentUser->setUserWriteAccess(AVUser::getCurrentUser(), true);
                self::$lastCurrentUser = clone AVUser::getCurrentUser();
            }
