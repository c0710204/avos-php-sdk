...
/**
 * AVQuery - Handles querying data from Avos
 *
 * @package  Avos
...
class AVQuery {
    ...
    /**
     * @var - Class Name for data stored on Avos.
     */
    private $className;
    /**
    ...
    /**
     * Create a Avos Query for a given Avos Class.
     *
     * @param mixed $className Class Name of data on Avos.
     */
    ...
    public function get($objectId, $useMasterKey = false)
    {
        ...
        if (empty($result))
        {
            throw new AVException("Object not found.", 101);
        }
     ...
     *
     * @return AVQuery Returns this query, so you can chain this call.
     */
    ...
    private function addCondition($key, $condition, $value)
    {
        ...
        $this->where[$key][$condition] = AVClient::_encode($value, true);
    }
    ...
    private function buildQueryString($queryOptions)
    {
        ...
        {
            $queryOptions["where"] = AVClient::_encode($queryOptions["where"], true);
    ...
    public function count($useMasterKey = false)
    {
        ...
        $result = AVClient::_request('GET', '/classes/' . $this->className . '?' . $queryString, null, null,
            $useMasterKey);
    ...
    public function find($useMasterKey = false)
    {
        ...
        $queryString = $this->buildQueryString($this->_getOptions());
        $result = AVClient::_request('GET', '/classes/' . $this->className . '?' . $queryString, $sessionToken, null,
            $useMasterKey);
        $output = array();
     ...
     *
     * @param string $key The key that the AVGeoPoint is stored in.
     * @param AVGeoPoint $point The reference AVGeoPoint that is used.
     *
     * @return AVQuery Returns this query, so you can chain this call.
     */
     ...
     *
     * @param string $key The key of the AVGeoPoint
     * @param AVGeoPoint $point The AVGeoPoint that is used.
     ...
     *
     * @param string $key The key of the AVGeoPoint
     * @param AVGeoPoint $southwest The lower-left corner of the box.
     * @param AVGeoPoint $northeast The upper-right corner of the box.
     *
     * @return AVQuery Returns this query, so you can chain this call.
     */
    public function withinGeoBox($key, $southwest, $northeast)
    {
        $this->addCondition($key, '$within', ['$box' => [$southwest, $northeast]]);
     ...
     *
     * @return AVQuery Returns the query, so you can chain this call.
     */
    ...
    public function each($callback, $useMasterKey = false, $batchSize = 100)
    {
        ...
        $query = new AVQuery($this->className);
    ...
    /**
     * Add a constraint that requires that a key's value matches a AVQuery
     * constraint.
     *
     * @param string $key The key that the contains the object to match
     *                          the query.
     * @param AVQuery $query The query that should match.
     *
     * @return AVQuery Returns the query, so you can chain this call.
     */
    ...
    /**
     * Add a constraint that requires that a key's value not matches a AVQuery
     * constraint.
     *
     * @param string $key The key that the contains the object not to
     *                          match the query.
     * @param AVQuery $query The query that should not match.
     *
     * @return AVQuery Returns the query, so you can chain this call.
     */
     ...
     *                             match against.
     * @param AVQuery $query The query to run.
     *
     * @return AVQuery Returns the query, so you can chain this call.
     */
    public function matchesKeyInQuery($key, $queryKey, $query)
    {
        $queryParam = $query->_getOptions();
        $queryParam["className"] = $query->className;
        $this->addCondition($key, '$select', ['key' => $queryKey, 'query' => $queryParam]);
    ...
    /**
     * Restrict the fields of the returned Avos Objects to include only the
    ...
    /**
     * Include nested Avos Objects for the provided key.  You can use dot
    ...
    /**
     * Add constraint for AV relation.
     *
     * @param string $key
     * @param mixed $value
     *
