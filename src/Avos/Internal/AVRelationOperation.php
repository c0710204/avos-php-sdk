...
class AVRelationOperation implements FieldOperation {
    ...
    public function __construct($objectsToAdd, $objectsToRemove)
    {
        ...
        {
            throw new \Exception('Cannot create a AVRelationOperation with no objects.');
        }
    }

    /**
     * Helper function to check that all passed AVObjects have same class name
     * and assign targetClassName variable.
     *
     * @param array $objects AVObject array.
     *
     * @throws \Exception
     */
     ...
     *
     * @param array $objects Array of AVObjects to add.
     * @param array $container Array to contain new AVObjects.
     */
     ...
     *
     * @param array $objects Array of AVObjects to remove.
     * @param array $container Array to remove from it AVObjects.
     */
    ...
    public function _apply($oldValue, $object, $key)
    {
        if ($oldValue == null)
        {
            return new AVRelation($object, $key, $this->targetClassName);
        } else if ($oldValue instanceof AVRelation)
        {
            if ($this->targetClassName != null && $oldValue->getTargetClass() !== $this->targetClassName)
            {
                throw new \Exception('Related object object must be of class ' . $this->targetClassName . ', but ' . $oldValue->getTargetClass() . ' was passed in.');
            }
    ...
    public function _encode()
    {
        ...
        if (!empty($this->relationsToAdd))
        {
            $addRelation = array(
                '__op'    => 'AddRelation',
                'objects' => AVClient::_encode(self::convertToOneDimensionalArray($this->relationsToAdd), true)
            );
        }
        if (!empty($this->relationsToRemove))
        {
            $removeRelation = array(
                '__op'    => 'RemoveRelation',
                'objects' => AVClient::_encode(self::convertToOneDimensionalArray($this->relationsToRemove), true)
            );
        }
