...
/**
 * AVFile - Representation of a Avos File object.
 *
 * @package  Avos
 * @author   Joe Chu <aidai524@gmail.com>
 */
class AVFile implements \Avos\Internal\Encodable {
    ...
    private $name;
    /**
     * @var - URL of File data stored on Avos.
     */
    private $url;
    /**
     * @var - Data
    ...
    public function getData()
    {
        ...
        if (!$this->url)
        {
            throw new AVException("Cannot retrieve data for unsaved AVFile.");
        }
     ...
     * Upon saving to Avos, the name will change to a unique identifier.
     *
     * @return string
     */
    ...
    public function delete()
    {
        if (!$this->url)
        {
            throw new AVException("Cannot delete file that has not been saved.");
        }

        $headers = AVClient::_getRequestHeaders(null, true);
    ...
    /**
     * Create a Avos File from data
     * i.e. $file = AVFile::createFromData("hello world!", "hi.txt");
     *
     * @param mixed $contents The file contents
     * @param string $name The file name on Avos, can be used to detect mimeType
    ...
    /**
     * Create a Avos File from the contents of a local file
     * i.e. $file = AVFile::createFromFile("/tmp/foo.bar",
     * "foo.bar");
     *
     * @param string $path Path to local file
     * @param string $name Filename to use on Avos, can be used to detect mimeType
     * @param string $mimeType Optional, The mime-type to use when saving the file
     *
     * @return AVFile
     */
    ...
    private function download()
    {
        ...
        if ($httpStatus > 399)
        {
            throw new AVException("Download failed, file may have been deleted.", $httpStatus);
        }
