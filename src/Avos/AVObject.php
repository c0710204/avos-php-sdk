...
class AVObject implements Encodable
{
                              ...
                              $isPointer = false)
  {
    if (empty(self::$registeredSubclasses)) {
      throw new Exception(
        'You must initialize the AVClient using AVClient::initialize ' .
        'and your Avos API keys before you can begin working with Objects.'
      );
    }
    ...
    if ($class !== __CLASS__ && $className !== $subclass) {
      throw new Exception(
        'You must specify a Avos class name or register the appropriate ' .
        'subclass when creating a new Object.  Use AVObject::create to ' .
  ...
  public function get($key)
  {
    if (!$this->_isDataAvailable($key)) {
      throw new \Exception(
          'AVObject has no data for this key. Call fetch() to get the data.');
    }
  ...
  public static function destroyAll(array $objects, $useMasterKey = false)
  {
    ...
    if ($count) {
      ...
      if (count($errors)) {
        throw new AVAggregateException(
  ...
  private static function deepSave($target, $useMasterKey = false)
  {
    ...
    while (count($remaining) > 0) {
      ...
      if (count($requests) === 1) {
        $req = $requests[0];
        $result = AVClient::_request($req['method'],
  ...
  public static function registerSubclass()
  {
    if (isset(static::$avClassName)) {
      if (!in_array(static::$avClassName, self::$registeredSubclasses)) {
        self::$registeredSubclasses[static::$avClassName] =
          get_called_class();
      }
    } else {
      throw new \Exception(
        "Cannot register a subclass that does not have a avClassName"
      );
    }
  }
