...
use Avos\Internal\RemoveOperation;
use Avos\Internal\FieldOperation;
use Avos\Internal\SetOperation;
use Avos\Internal\AddOperation;
use Avos\Internal\AddUniqueOperation;
use Avos\Internal\IncrementOperation;
use Avos\Internal\DeleteOperation;

use \Exception;

/**
 * AVObject - Representation of an object stored on Avos.
 *
 * @package  Avos
 * @author   Joe Chu <aidai524@gmail.com>
 */
class AVObject implements Encodable {
    ...
    /**
     * @var - Class Name for data on Avos.
     */
    private $className;
    /**
     * @var string - Unique identifier on Avos.
     */
    private $objectId;
    /**
    ...
    private $updatedAt;
    /**
     * @var bool - Whether the object has been fully fetched from Avos.
     */
    private $hasBeenFetched;

    /**
     * @var array - Holds the registered subclasses and Avos class names.
     */
    private static $registeredSubclasses = array();

    /**
     * Create a Avos Object
     *
     * Creates a pointer object if an objectId is provided,
     * otherwise creates a new object.
     *
     * @param string $className Class Name for data on Avos.
    ...
    public function __construct(
        ...
        {
            throw new Exception('You must initialize the AVClient using AVClient::initialize ' . 'and your Avos API keys before you can begin working with Objects.');
        }
        ...
        {
            throw new Exception('You must specify a Avos class name or register the appropriate ' . 'subclass when creating a new Object.  Use AVObject::create to ' . 'create a subclass object.');
        }
    ...
    public function get($key)
    {
        ...
        {
            throw new \Exception('AVObject has no data for this key. Call fetch() to get the data.');
        }
    ...
    /**
     * Get the AV Class Name for the object.
     *
     * @return string
     */
    ...
    /**
     * Static method which returns a new Avos Object for a given class
    ...
    public static function create(
        ...
        {
            return new AVObject($className, $objectId, $isPointer);
        }
    }
    ...
    public function fetch()
    {
        ...
        $response = AVClient::_request('GET', '/classes/' . $this->className . '/' . $this->objectId, $sessionToken);
    ...
    private function mergeFromServer($data, $completeData = true)
    {
        ...
        foreach ($data as $key => $value)
        {
            ...
            $decodedValue = AVClient::_decode($value);
            ...
            if (is_array($decodedValue))
            {
                if (isset($decodedValue['__type']))
                {
                    if ($decodedValue['__type'] === 'Relation')
                    {
                        ...
                        $decodedValue = new AVRelation($this, $key, $className);
                    }
                }
                if ($key == 'ACL')
                {
                    $decodedValue = AVACL::_createACLFromJSON($decodedValue);
                }
            }
    ...
    public function destroy($useMasterKey = false)
    {
        ...
        AVClient::_request('DELETE', '/classes/' . $this->className . '/' . $this->objectId, $sessionToken, null,
            $useMasterKey);
    }
    ...
    public static function destroyAll(array $objects, $useMasterKey = false)
    {
        ...
        if ($count)
        {
            ...
            if (count($errors))
            {
                throw new AVAggregateException("Errors during batch destroy.", $errors);
            }
        }
    ...
    private static function destroyBatch(array $objects, $useMasterKey = false)
    {
        ...
        $result = AVClient::_request("POST", "/batch", $sessionToken, json_encode(array("requests" => $data)),
            $useMasterKey);
    ...
    public function _encode()
    {
        ...
        foreach ($this->estimatedData as $key => $value)
        {
            if (is_object($value) && $value instanceof \Avos\Internal\Encodable)
            {
            ...
            } else if (is_array($value))
            {
                ...
                {
                    if (is_object($item) && $item instanceof \Avos\Internal\Encodable)
                    {
    ...
    private static function deepSave($target, $useMasterKey = false)
    {
        ...
        while (count($remaining) > 0)
        {
            ...
            {
                $req = $requests[0];
                $result = AVClient::_request($req['method'], $req['path'], $sessionToken, json_encode($req['body']),
                    $useMasterKey);
                $batch[0]->mergeAfterSave($result);
            } else
            {
                $result = AVClient::_request('POST', '/batch', $sessionToken,
                ...
                {
                    throw new AVAggregateException("Errors during batch save.", $errorCollection);
                }
            }
        }
    }
    ...
    private static function traverse(
        ...
        if ($object instanceof AVRelation || $object instanceof AVFile)
        {
            return $mapFunction($object);
        }
        if (is_array($object))
     ...
     * @return AVRelation The AVRelation object if the relation already
     *                       exists for the key or can be created for this key.
     */
    public function getRelation($key)
    {
        $relation = new AVRelation($this, $key);
        if (isset($this->estimatedData[$key]))
        {
    ...
    public function _toPointer()
    {
        if (!$this->objectId)
        {
            throw new \Exception("Can't serialize an unsaved Avos.Object");
        }
    ...
    /**
     * Register a subclass.  Should be called before any other Avos functions.
     * Cannot be called on the base class AVObject.
     *
     * @throws \Exception
     */
    public static function registerSubclass()
    {
        if (isset(static::$avClassName))
        {
            if (!in_array(static::$avClassName, self::$registeredSubclasses))
            {
                self::$registeredSubclasses[static::$avClassName] = get_called_class();
            }
        } else
        {
            throw new \Exception("Cannot register a subclass that does not have a avClassName");
        }
    }
    ...
    /**
     * Creates a AVQuery for the subclass of AVObject.
     * Cannot be called on the base class AVObject.
     *
     * @return AVQuery
     *
